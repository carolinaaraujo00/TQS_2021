...
public class Dip {
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (LENGTH_NUMBERS == arrayOfNumbers.length && LENGTH_STARTS == arrayOfStarts.length) {
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < LENGTH_NUMBERS; i++) {
            int candidate = generator.nextInt(LIMIT_NUMBERS) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
            }
        }

        for (int i = 0; i < LENGTH_STARTS; i++) {
            int candidate = generator.nextInt(LIMIT_STARTS) + 1;
